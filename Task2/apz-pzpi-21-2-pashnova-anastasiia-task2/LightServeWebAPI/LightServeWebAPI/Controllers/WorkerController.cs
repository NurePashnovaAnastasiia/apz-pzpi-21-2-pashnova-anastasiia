...
namespace LightServeWebAPI.Controllers
{
    ...
    public class WorkerController : ControllerBase
    {
        ...
        [HttpPost("login")]
        public async Task<ActionResult<Worker>> Login(string username, string password)
        {
            if (!string.IsNullOrWhiteSpace(username))
            {
                var worker = await _workerRepository.GetWorkerByUsername(username);
        ...
        public async Task<ActionResult<Worker>> ResetPassword(string username, string password)
        {
            if (!string.IsNullOrWhiteSpace(username))
            {
                var worker = await _workerRepository.GetWorkerByUsername(username);
